...
?>
	...
	<?php
      ...
      ?>
		<ul>
			...
			<h2 style="font-style:italic; font-size:30px;text-align: center;">View Medicine</h2>
	...
	<?php
		...
		$ret=mysqli_query($conn,$sql);
            ...
            {
                    echo"<div id='dem'>";
					echo"<table border='1'><tr><th>Medicine Name</th><th>Quantity</th><th>Expiry Date</th><th>Amount</th><th>Rack number</th></tr>";
					while($row=mysqli_fetch_assoc($ret))
					{
						echo"<tr><td>{$row['medicine_name']}</td><td>{$row['quanity']}</td><td>{$row['expiry_date']}</td><td>{$row['amount']}</td><td>{$row['rack_number']}</td></tr>";
            ...
            if(mysqli_num_rows($ret)==0)
            {
                      echo "<div id='card'><h1>No Medicine of such name  is there....</h1><form action='pharmacist_home.php'><button type='submit' id='done'>Done</button></form></div>";
